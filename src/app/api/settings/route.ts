  ...
  try {
    ...
    return NextResponse.json({
      settings: {
        ...
        dailyQuizCount: settings.dailyQuizCount ?? 10,
        learningLevel: settings.learningLevel ?? "intermediate",
        contentFocus: settings.contentFocus ?? "quiz",
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    create: {
      ...
      dailyQuizCount: dailyQuizCount ?? 10,
      learningLevel: (learningLevel as (typeof LEARNING_LEVELS)[number]) ?? "intermediate",
      contentFocus: (contentFocus as (typeof CONTENT_FOCUS)[number]) ?? "quiz",
